...
class DoclingBookLoader:
    ...
    def __init__(
        ...
        Args:
            ...
            text_only: If True, use lightweight text extraction (no OCR, layout, tables)
        """
        ...
        # In text-only mode, disable heavy processing
        if text_only:
            do_ocr = False
            do_table_structure = False
            num_threads = 1  # Reduce threads for lightweight processing
    ...
    def extract_structured_content(self) -> Dict[str, Any]:
        ...
        try:
            ...
            metadata = {
                ...
                "text_only_mode": self.text_only,
            }

            # In text-only mode, skip heavy extraction operations
            if self.text_only:
                return {
                    "text": doc.export_to_markdown(),
                    "full_text": doc.export_to_markdown(),  # Alias for compatibility
                    "raw_document": doc,
                    "metadata": metadata,
                    "tables": [],  # Empty in text-only mode
                    "images": []   # Empty in text-only mode
                }
            else:
                return {
                    "text": doc.export_to_markdown(),
...
class LoaderFactory:
    ...
    def create_loader(file_path: str, **kwargs) -> DoclingBookLoader:
        ...
        Args:
            ...
            **kwargs: Additional arguments for loader (including text_only)

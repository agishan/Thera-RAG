...
class EnhancedVectorUploader:
    ...
    def __init__(
        ...
        # Metadata size controls (optional)
        # Env var PINECONE_STORE_FULL_TEXT=false will store preview instead of full text
        env_store_full = os.getenv("PINECONE_STORE_FULL_TEXT")
        self.store_full_text = True if env_store_full is None else env_store_full.lower() in ("1", "true", "yes")
        self.text_preview_chars = int(os.getenv("TEXT_PREVIEW_CHARS", "2000"))
    ...
    def _prepare_metadata_for_pinecone(self, metadata: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Store text content with optional preview
        text_content = metadata.get('text', '')
        if self.store_full_text:
            pinecone_metadata['text'] = text_content
        else:
            pinecone_metadata['text_preview'] = text_content[: self.text_preview_chars]
        ...
        # Handle complex fields (always include)
        self._add_complex_fields(metadata, pinecone_metadata)

        # Add derived/computed fields for search (always include)
